...
namespace IO
{
    ...
    {   
        ...
        /// <summary>
        /// Default constructor.
        /// Her bliver _connectionString initialiseret til en gyldig connectionstring
        /// med data omkring URL til Sql-Serveren og information omkring hvilke 
        /// DataBase der skal benyttes på serveren.
        ...
        /// Har samme funktion som den default construktor, men skal modtage en parameter
        /// med angivelse af connection til Sql-Serveren via en ConnectionString.
        /// Yderligere info omkring indhold connectionstring findes her: 
        /// https://www.connectionstrings.com/sql-server/
        /// </summary>
        ...
        /// Metoden kan kaldes efter behov og vil overroule den SqlConnection som
        /// blev oprettet i en af de to construkteres.
        /// Yderligere info omkring indhold connectionstring findes her: 
        /// https://www.connectionstrings.com/sql-server/
        /// </summary>
        /// <param name="inConnectionString">string</param>
        ...
        protected void OpenDB()
        {
            try
            {
                ...
                {
                    if (con.State == ConnectionState.Open) // Undersøger om fejlen skyldes om der en åben forbindelse i forvejen.
                    {
                        // Hvis ja - Lukker forbindelsen og kalder 'sig selv' igen for at åbne forbindelsen.
                        CloseDB();
                        OpenDB();
                    }
                    else // Hvis det ikke var på grund af en åben forbindelse, så må det være på grund af manglende initialisering af 'con'
                    {
        ...
        protected int ExecuteNonQuery(string sqlQuery)
        {
            ...
            return res; // Returnere resultat af forespørgelsen
        }
        ...
        protected DataTable DbReturnDataTable(string sqlQuery)
        {
            ...
            try
            {
                ...
                {
                    using (var adapter = new SqlDataAdapter(_command)) // Her foretages kaldet til databasen ved, at der oprettes en ny instans af en SqlDataAdapter.
                                                                       // Resultatet overføres til en abstrakt datatype.
                    {
            ...
            return dtRes; // Returnere resultat af forespørgelsen
        }
        ...
        protected string DbReturnString(string sqlQuery)
        {
            ...
            try
            {
                                                                       ...
                                                                       // hvilken database data skal hentes fra.
                {
                    ...
                    if (!foundOne)
                    {
                        res = "No data found!"; // Hvis der ikke findes et resultat i databasen, skal der returneres en sigende tekst.
                    }
                }
            }
            catch (SqlException ex)
